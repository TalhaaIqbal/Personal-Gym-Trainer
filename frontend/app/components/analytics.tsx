            ...
            <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
              ...
              <div className="space-y-2">
                {bookingTrends.map((trend) => (
                  <div key={trend.date} className="flex items-center justify-between bg-white/5 rounded-lg p-3">
                    <span className="text-blue-200">{new Date(trend.date).toLocaleDateString()}</span>
                    <div className="flex space-x-4">
                      <span className="text-green-400">Confirmed: {trend.confirmed}</span>
                      <span className="text-yellow-400">Pending: {trend.pending}</span>
                      <span className="text-red-400">Cancelled: {trend.cancelled}</span>
                      <span className="text-white font-bold">Total: {trend.total}</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/20">
                      <th className="text-left p-3 text-blue-200">Client Name</th>
                      <th className="text-left p-3 text-blue-200">Email</th>
                      <th className="text-center p-3 text-blue-200">Total Sessions</th>
                      <th className="text-center p-3 text-blue-200">Completed</th>
                      <th className="text-center p-3 text-blue-200">Cancelled</th>
                      <th className="text-center p-3 text-blue-200">Completion Rate</th>
                      <th className="text-center p-3 text-blue-200">Last Session</th>
                    </tr>
                  </thead>
                  <tbody>
                    {clientStats.map((client) => (
                      <tr key={client.client_id} className="border-b border-white/10 hover:bg-white/5">
                        <td className="p-3 text-white">{client.client_name}</td>
                        <td className="p-3 text-blue-200">{client.client_email}</td>
                        <td className="p-3 text-center text-white font-bold">{client.total_sessions}</td>
                        <td className="p-3 text-center text-green-400">{client.completed_sessions}</td>
                        <td className="p-3 text-center text-red-400">{client.cancelled_sessions}</td>
                        <td className="p-3 text-center text-white">{client.completion_rate}%</td>
                        <td className="p-3 text-center text-blue-200">
              ...
              <div className="space-y-3">
                {timeSlots.map((slot) => (
                  <div key={slot.hour} className="flex items-center space-x-4">
                    <span className="text-blue-200 w-24">{slot.time_label}</span>
                    ...
                    <span className="text-white font-bold w-16 text-right">{slot.count} sessions</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
              ...
              <div className="space-y-3">
                {revenueData.map((month) => (
                  <div key={month.month} className="flex items-center justify-between bg-white/5 rounded-lg p-4">
                    <div>
